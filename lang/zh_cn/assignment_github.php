<?php

$string['choosegroup'] = '请先切换到一个小组';
$string['deletealllogs'] = '删除所有容器日志';
$string['deleteallrepos'] = '删除所有容器';
$string['deleteallreposettings'] = '删除所有容器设置';
$string['editrepository'] = '编辑容器';
$string['editusername'] = '编辑Username或organization';
$string['editusername_help'] = '容器所有者的Username或organization。';
$string['emailnotset'] = '您未设置向git提交所用的邮箱';
$string['githubconfigureguide1'] = '为完成配置，请确认您在github.com上是组织 {$a} 的管理员之一。登入github.com 然后点击下面的链接：';
$string['githubconfigureguide2'] = '若需要更改授权用户，请使用另外的账户登录github.com 然后点击下面的链接：';
$string['githubconfigureguide3'] = '将{$a}添加到组织中，并确认它有权限访问所有容器。';
$string['githubreposetting'] = 'Git容器';
$string['githubreposettinglist'] = '所有容器列表';
$string['githubusername'] = 'GitHub用户名';
$string['lastsynced'] = '最近同步';
$string['latestcommits'] = '最近的提交';
$string['memberemail'] = '{$a}的email';
$string['memberemail_help'] = '此用户在此git容器上提交代码所用的邮箱';
$string['memberlist'] = '成员列表';
$string['pluginname'] = 'Github项目';
$string['private'] = '私有';
$string['project'] = '项目';
$string['projecthome'] = '项目首页';
$string['publicorprivate'] = '公开/私有';
$string['public'] = '公开';
$string['repohasnotset'] = '此用户或分组的容器尚未设置';
$string['repohasset'] = '{$a} 个容器已设置';
$string['reponotfind'] = '未在托管服务器上找到指定的容器。请检查链接是否输入正确。';
$string['repository'] = '容器';
$string['repositoryname'] = '容器名称';
$string['repositoryrourl'] = '容器只读URL';
$string['repositoryrourl_help'] = 'Git容器的只读链接。例如：git://github.com/username/projectname.git';
$string['serviceerror'] = '服务出错。请检查链接是否输入正确。如果仍旧发生此错误，请联系站点管理员。';
$string['statistics'] = '统计数据';
$string['statisticsdata'] = '{$a} 的统计数据';
$string['submitnotallowed'] = '在教师完成配置之前，你不能提交作业';
$string['typegithub'] = 'Github项目';
$string['unknownserver'] = '无法识别的托管服务器。请检查链接是否输入正确。';
$string['unknowntype'] = '未知的类型参数';
$string['unrecognizedurl'] = '无法识别的链接。请检查链接是否输入正确。';
$string['viewrepolist'] = '所有容器列表';
